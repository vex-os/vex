...
static size_t write_int(format_func_t func, void* restrict user, uintmax_t value, unsigned int flags, unsigned int width,
    unsigned int precision, unsigned int base)
{
    unsigned int i;
    size_t stager;
    size_t counter;
...
static size_t write_str(format_func_t func, void* restrict user, const char* restrict s, unsigned int flags, unsigned int width,
...
static unsigned int get_flag(int character)
{
    switch(character) {
        case '0':
            return F_ZERO_PAD;
...
int format(format_func_t func, void* restrict user, const char* restrict fmt, ...)
{
    int r;
    va_list ap;
    va_start(ap, fmt);
    r = vformat(func, user, fmt, ap);
    va_end(ap);
    return r;
}

int vformat(format_func_t func, void* restrict user, const char* restrict fmt, va_list ap)
{
    size_t counter = 0;
    unsigned int width;
    unsigned int precision;
    ...
    while(*fmt) {
        ...
        width = strtoul(fmt, &endptr, 10);
        precision = 0U;
        fmt = endptr;

        if(*fmt == '.') {
            precision = strtoul(++fmt, &endptr, 10);
            fmt = endptr;
        }
        ...
        switch(*fmt) {
            ...
            case 'i':
                ...
                counter += write_int(func, user, value, flags, width, precision, 10);
                break;
            ...
            case 'x':
                counter += write_int(func, user, value, flags, width, precision, 16);
                break;
            ...
            case 'o':
                counter += write_int(func, user, value, flags, width, precision, 8);
                break;

            case 'b':
                counter += write_int(func, user, value, flags, width, precision, 2);
                break;
            ...
            case 's':
                counter += write_str(func, user, va_arg(ap, const char*), flags, width, precision);
                break;
            ...
            case 'p':
                ...
                flags |= F_ZERO_PAD;
                flags |= F_UPPERCASE;
                counter += write_int(func, user, va_arg(ap, uintptr_t), flags, sizeof(uintptr_t) * 2, precision, 16);
                break;
