...
struct vmm_pagemap *vmm_create(void)
{
    ...
    if((pagemap = kmalloc(sizeof(struct vmm_pagemap))) != NULL) {
        if((pagemap->vm_ptr = pmm_alloc(1)) != 0) {
...
static void init_vmm(void)
{
    ...
    sys_pagemap = kmalloc(sizeof(struct vmm_pagemap));
    kassertf(sys_pagemap, "vmm: insufficient memory");

    sys_pagemap->vm_ptr = pmm_alloc(1);
    ...
    for(i = 0; i < memmap_request.response->entry_count; ++i) {
        ...
        if(entry->type == LIMINE_MEMMAP_BAD_MEMORY)
            continue;
        vstart = page_align_address(entry->base) + highmem;
