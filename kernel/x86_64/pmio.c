/* SPDX-License-Identifier: BSD-2-Clause */
/* Copyright (c), 2022, Kaneru Contributors */
#include <kaneru/cdefs.h>
#include <kaneru/errno.h>
#include <x86_64/pmio.h>

#define X86_MAX_PMIO_OFF 0xFFFF

int x86_pmio_read8(uintptr_t off, uint8_t *restrict val)
{
    if(off > X86_MAX_PMIO_OFF)
        return -EIO;
    asm volatile("inb %1, %0":"=a"(val[0]):"Nd"((uint16_t)(off)));
    return 0;
}

int x86_pmio_read16(uintptr_t off, uint16_t *restrict val)
{
    if(off > X86_MAX_PMIO_OFF)
        return -EIO;
    asm volatile("inw %1, %0":"=a"(val[0]):"Nd"((uint16_t)(off)));
    return 0;
}

int x86_pmio_read32(uintptr_t off, uint32_t *restrict val)
{
    if(off > X86_MAX_PMIO_OFF)
        return -EIO;
    asm volatile("inl %1, %0":"=a"(val[0]):"Nd"((uint16_t)(off)));
    return 0;
}

int x86_pmio_write8(uintptr_t off, uint8_t val)
{
    if(off > X86_MAX_PMIO_OFF)
        return -EIO;
    asm volatile("outb %0, %1"::"a"(val), "Nd"((uint16_t)(off)));
    return 0;
}

int x86_pmio_write16(uintptr_t off, uint16_t val)
{
    if(off > X86_MAX_PMIO_OFF)
        return -EIO;
    asm volatile("outw %0, %1"::"a"(val), "Nd"((uint16_t)(off)));
    return 0;
}

int x86_pmio_write32(uintptr_t off, uint32_t val)
{
    if(off > X86_MAX_PMIO_OFF)
        return -EIO;
    asm volatile("outl %0, %1"::"a"(val), "Nd"((uint16_t)(off)));
    return 0;
}

void x86_pmio_throttle(void)
{
    /* FIXME: this may not work on certain
     * platforms as this port is taken by some
     * device that shits bricks when startled. */
    asm volatile("outb %0, $0x80"::"a"((uint8_t)(0)));
}
