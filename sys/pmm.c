...
uintptr_t pmm_alloc(size_t npages)
{
    ...
    // FIXME: figure something out, do not crash that easily.
    panic("pmem: insufficient memory (failed to allocate %zu pages)", npages);
    UNREACHABLE();
    return 0;
}
...
{
    uintptr_t address = pmm_alloc(npages);
    if(address)
